...
""")
    ...
    with st.expander("탭 1·2에 보이는 ①②(추출·관계 생성)는 지금 이 순간 실시간으로 만들어지는 건가요?"):
        st.markdown(
            "아니요. **①②는 이 프로젝트를 준비할 때 미리 한 번 돌려서 그 결과를 파일로 저장해둔 것**입니다. "
            "화면을 열 때마다 AI를 다시 호출해서 새로 만드는 게 아니라, 저장된 결과를 읽어서 보여줄 뿐입니다. "
            "이 화면(웹앱)이 실시간으로 하는 건 **③사람 검증**과 **④그래프 확인**뿐입니다.\n\n"
            "맨 위 **\"🔑 API 키로 실제 LLM 추출 테스트\"** 섹션은 별개의 실험용 기능입니다 — 본인 API 키로 "
            "실시간 ①추출을 체험해볼 수는 있지만, 그 결과가 탭 1·2의 관계 후보 목록에 자동으로 이어지지는 "
            "않습니다 (②관계 생성까지 연결하려면 개발자가 별도 스크립트를 로컬에서 실행해야 합니다)."
        )
